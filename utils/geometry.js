...
//v1 + v2*l2
export function add(v1, v2, l2 = 1) {
    return v1.map((v1v, idx) => { return v1[idx] + v2[idx] * l2 });
}

export function average(vs, ws = undefined) {
    let weight = (ws == undefined) ? vs.length : ws.reduce((previousValue, v) => { return previousValue + v; }, 0);

    if (weight == 0) {
        return undefined;
    }

    return vs[0].map((v1v, idx) => {
        return vs.reduce((previousValue, v, widx) => {
            if (ws == undefined) {
                return previousValue + v[idx];
            } else {
                return previousValue + v[idx] * ws[widx];
            }
        }, 0) / weight
    });
}
